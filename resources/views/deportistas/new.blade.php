...
@section('contenido')
<form action="{{ route('deportistas.store') }}" method="POST" id="formNuevoDeportista"
      style="max-width: 800px; margin: auto; font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ccc; border-radius: 8px;">
    @csrf
    <h1 class="text-center mb-4">Registrar Nuevo Deportista</h1>
        ...
        <div class="col-md-6 mb-3">
            ...
            <select name="IdDisciplina" id="IdDisciplina" class="form-select" required>
                ...
                @foreach($disciplinas as $disciplina)
                    <option value="{{ $disciplina->IdDisciplina }}">{{ $disciplina->NombreDisciplina }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            ...
            <input type="number" name="Estatura" id="Estatura" class="form-control" step="0.01" min="0.5" max="2.5" required>
        </div>

        <div class="col-md-4 mb-3">
            <label for="Peso" class="form-label fw-bold">Peso (kg):</label>
            <input type="number" name="Peso" id="Peso" class="form-control" step="0.1" min="20" max="200" required>
        </div>
    </div>
