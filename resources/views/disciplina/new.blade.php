...
@section('contenido')
<form action="{{ route('disciplina.store') }}" method="POST" id="formNuevaDisciplina"
      style="max-width: 700px; margin: auto; font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ccc; border-radius: 8px;">
    @csrf
    <h1 class="text-center mb-4">Registrar Nueva Disciplina</h1>

    <div class="mb-3">
        <label for="NombreDisciplina" class="form-label fw-bold">Nombre de la Disciplina:</label>
        <input type="text" name="NombreDisciplina" id="NombreDisciplina" class="form-control" required>
    </div>
